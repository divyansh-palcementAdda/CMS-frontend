...
export class GenderDetailComponent implements OnInit, OnDestroy {
  ...
  Math = Math;
  genderId!: string;
  genderDetail: GenderDetailDTO | null = null;
  loading = true;
  ...
  loadAll(): void {
    this.loading = true;
    this.locationAnalyticsService.getGenderDetail(this.genderId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (data) => {
        this.genderDetail = data;
        this.loading = false;
      ...
      error: (err) => {
        console.error('Error loading gender detail:', err);
        this.loading = false;
      }
    });
  }
  ...
  loadStudents(tab: string): void {
    ...
    this.locationAnalyticsService.getGenderStudentsPaged(
      this.genderId, page, size, tab, search, source || '', scholar, sortBy, sortDir, duplicateOnly, showOnlyFoc, showOnlySbs
      ...
      error: err => console.error(`Error loading gender students (${tab}):`, err)
    });
  }

  // ---- User Breakdown ----
  loadUserBreakdown(): void {
    this.locationAnalyticsService.getGenderUserBreakdown(this.genderId, this.userPage - 1, this.userPageSize, this.userSortBy, this.userSortDir)
      .pipe(takeUntil(this.destroy$)).subscribe({
        next: (res) => { this.userBreakdownList.set(res.content || []); this.userTotal = res.totalElements || 0; },
        error: err => console.error('Error loading gender user breakdown:', err)
      });
  }

  // ---- Consultancy Breakdown ----
  loadConsultancyBreakdown(): void {
    this.locationAnalyticsService.getGenderConsultancyBreakdown(this.genderId, this.consPage - 1, this.consPageSize, this.consSortBy, this.consSortDir)
      .pipe(takeUntil(this.destroy$)).subscribe({
        next: (res) => { this.consultancyList.set(res.content || []); this.consTotal = res.totalElements || 0; },
        error: err => console.error('Error loading gender consultancy breakdown:', err)
      });
  }

  // ---- Institution Breakdown ----
  loadInstitutionBreakdown(): void {
    this.locationAnalyticsService.getGenderInstitutionBreakdown(this.genderId, this.instPage - 1, this.instPageSize, this.instSortBy, this.instSortDir)
      .pipe(takeUntil(this.destroy$)).subscribe({
        next: (res) => { this.institutionList.set(res.content || []); this.instTotal = res.totalElements || 0; },
        error: err => console.error('Error loading gender institution breakdown:', err)
      });
  }

  // ---- Course Breakdown ----
  loadCourseBreakdown(): void {
    this.locationAnalyticsService.getGenderCourseBreakdown(this.genderId, this.coursePage - 1, this.coursePageSize, this.courseSortBy, this.courseSortDir)
      .pipe(takeUntil(this.destroy$)).subscribe({
        next: (res) => { this.courseList.set(res.content || []); this.courseTotal = res.totalElements || 0; },
        error: err => console.error('Error loading gender course breakdown:', err)
      });
  }

  // ---- Course Type Breakdown ----
  loadCourseTypeBreakdown(): void {
    this.locationAnalyticsService.getGenderCourseTypeBreakdown(this.genderId)
      .pipe(takeUntil(this.destroy$)).subscribe({
        next: (res) => this.courseTypeList = res || [],
        error: err => console.error('Error loading gender course type breakdown:', err)
      });
  }

  // ---- Lead Source Breakdown ----
  loadLeadSourceBreakdown(): void {
    this.locationAnalyticsService.getGenderLeadSourceBreakdown(this.genderId)
      .pipe(takeUntil(this.destroy$)).subscribe({
        next: (res) => this.leadSourceList = res || [],
        error: err => console.error('Error loading gender lead source breakdown:', err)
      });
  }
  ...
  // ---- Actions ----
  goBack(): void { this.router.navigate(['/location-analytics/genders']); }
  ...
  exportStudents(tab: string): void {
    ...
    this.locationAnalyticsService.exportGenderStudents(
      this.genderId, tab, search, source, scholar, duplicateOnly, showOnlyFoc, showOnlySbs
    ).subscribe({
      next: (blob) => { this.downloadBlob(blob, `Gender_${this.genderId}_${tab}_Students.xlsx`); this.exporting[key] = false; },
  ...
  exportUsers(): void {
    ...
    this.locationAnalyticsService.exportGenderUsers(this.genderId, undefined, this.userSortBy, this.userSortDir).subscribe({
      next: (blob) => { this.downloadBlob(blob, `Gender_${this.genderId}_Users.xlsx`); this.exporting['users'] = false; },
  ...
  exportConsultancies(): void {
    ...
    this.locationAnalyticsService.exportGenderConsultancies(this.genderId, undefined, this.consSortBy, this.consSortDir).subscribe({
      next: (blob) => { this.downloadBlob(blob, `Gender_${this.genderId}_Consultancies.xlsx`); this.exporting['consultancies'] = false; },
  ...
  exportInstitutions(): void {
    ...
    this.locationAnalyticsService.exportGenderInstitutions(this.genderId, undefined, this.instSortBy, this.instSortDir).subscribe({
      next: (blob) => { this.downloadBlob(blob, `Gender_${this.genderId}_Institutions.xlsx`); this.exporting['institutions'] = false; },
  ...
  exportCourses(): void {
    ...
    this.locationAnalyticsService.exportGenderCourses(this.genderId, undefined, this.courseSortBy, this.courseSortDir).subscribe({
      next: (blob) => { this.downloadBlob(blob, `Gender_${this.genderId}_Courses.xlsx`); this.exporting['courses'] = false; },
  ...
  exportCourseTypes(): void {
    ...
    this.locationAnalyticsService.exportGenderCourseTypes(this.genderId).subscribe({
      next: (blob) => { this.downloadBlob(blob, `Gender_${this.genderId}_CourseTypes.xlsx`); this.exporting['courseTypes'] = false; },
  ...
  exportLeadSources(): void {
    ...
    this.locationAnalyticsService.exportGenderLeadSources(this.genderId).subscribe({
      next: (blob) => { this.downloadBlob(blob, `Gender_${this.genderId}_LeadSources.xlsx`); this.exporting['leadSources'] = false; },
