...
@Component({
  selector: 'app-location-analytics-cities',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './location-analytics-cities.component.html',
  styleUrl: './location-analytics-cities.component.scss'
})
export class LocationAnalyticsCitiesComponent implements OnInit, OnDestroy {
  // Cities analytics list
  citiesData: LocationAnalyticsDTO[] = [];
  ...
  // Filters mapping exactly to AdmissionPageRequest fields
  filters: any = {
    search: '',
    statusFilter: '',
    states: [],
  ...
  // Search filter options inside dropdowns
  searchTerms = {
    session: '',
    admissionType: '',
    leadSource: '',
  ...
  ngOnInit(): void {
    ...
    // 2. Parse query parameters and load data
  ...
  loadData(): void {
    ...
    this.locationAnalyticsService.getCitiesAnalytics(requestPayload).subscribe({
      next: (res) => {
      ...
      error: (err) => {
        console.error('Error loading cities analytics', err);
        this.loading = false;
      }
    });
  }
  ...
  toggleSort(column: string): void {
    if (this.sortBy === column) {
      ...
      this.sortBy = column;
      this.sortDirection = 'desc'; // Default to desc for rankings
    }
    this.currentPage = 1;
    this.applyFiltersAndReload();
  }
  ...
  placeholderDownload(): void {
    alert('Excel Report Download placeholder triggered.');
  }
