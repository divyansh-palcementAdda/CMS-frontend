...
@Component({
  selector: 'app-location-analytics-states',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './location-analytics-states.component.html',
  styleUrl: './location-analytics-states.component.scss'
})
export class LocationAnalyticsStatesComponent implements OnInit, OnDestroy {
  // States analytics list
  statesData: LocationAnalyticsDTO[] = [];
  ...
  loadData(): void {
    ...
    this.locationAnalyticsService.getStatesAnalytics(requestPayload).subscribe({
      next: (res) => {
        this.statesData = res.content || [];
        this.totalElements = res.totalElements || 0;
        this.totalPages = res.totalPages || 0;
        this.loading = false;
      },
      error: (err) => {
        console.error('Error loading states analytics', err);
        this.loading = false;
      }
    });
  }
