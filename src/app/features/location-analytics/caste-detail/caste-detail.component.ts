...
@Component({
  ...
  templateUrl: './caste-detail.component.html',
  styleUrls: ['./caste-detail.component.scss']
})
export class CasteDetailComponent implements OnInit, OnDestroy {

  Math = Math;
  casteId!: string;
  casteDetail: CasteDetailDTO | null = null;
  loading = true;
  ...
  loadAll(): void {
    this.loading = true;
    this.locationAnalyticsService.getCasteDetail(this.casteId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (data) => {
        this.casteDetail = data;
        this.loading = false;
      ...
      error: (err) => {
        console.error('Error loading caste detail:', err);
        this.loading = false;
      }
    });
  }
  ...
  loadStudents(tab: string): void {
    ...
    this.locationAnalyticsService.getCasteStudentsPaged(
      this.casteId, page, size, tab, search, source || '', scholar, sortBy, sortDir, duplicateOnly, showOnlyFoc, showOnlySbs
      ...
      error: err => console.error(`Error loading caste students (${tab}):`, err)
    });
  }

  // ---- User Breakdown ----
  loadUserBreakdown(): void {
    this.locationAnalyticsService.getCasteUserBreakdown(this.casteId, this.userPage - 1, this.userPageSize, this.userSortBy, this.userSortDir)
      .pipe(takeUntil(this.destroy$)).subscribe({
        next: (res) => { this.userBreakdownList.set(res.content || []); this.userTotal = res.totalElements || 0; },
        error: err => console.error('Error loading caste user breakdown:', err)
      });
  }

  // ---- Consultancy Breakdown ----
  loadConsultancyBreakdown(): void {
    this.locationAnalyticsService.getCasteConsultancyBreakdown(this.casteId, this.consPage - 1, this.consPageSize, this.consSortBy, this.consSortDir)
      .pipe(takeUntil(this.destroy$)).subscribe({
        next: (res) => { this.consultancyList.set(res.content || []); this.consTotal = res.totalElements || 0; },
        error: err => console.error('Error loading caste consultancy breakdown:', err)
      });
  }

  // ---- Institution Breakdown ----
  loadInstitutionBreakdown(): void {
    this.locationAnalyticsService.getCasteInstitutionBreakdown(this.casteId, this.instPage - 1, this.instPageSize, this.instSortBy, this.instSortDir)
      .pipe(takeUntil(this.destroy$)).subscribe({
        next: (res) => { this.institutionList.set(res.content || []); this.instTotal = res.totalElements || 0; },
        error: err => console.error('Error loading caste institution breakdown:', err)
      });
  }

  // ---- Course Breakdown ----
  loadCourseBreakdown(): void {
    this.locationAnalyticsService.getCasteCourseBreakdown(this.casteId, this.coursePage - 1, this.coursePageSize, this.courseSortBy, this.courseSortDir)
      .pipe(takeUntil(this.destroy$)).subscribe({
        next: (res) => { this.courseList.set(res.content || []); this.courseTotal = res.totalElements || 0; },
        error: err => console.error('Error loading caste course breakdown:', err)
      });
  }

  // ---- Course Type Breakdown ----
  loadCourseTypeBreakdown(): void {
    this.locationAnalyticsService.getCasteCourseTypeBreakdown(this.casteId)
      .pipe(takeUntil(this.destroy$)).subscribe({
        next: (res) => this.courseTypeList = res || [],
        error: err => console.error('Error loading caste course type breakdown:', err)
      });
  }

  // ---- Lead Source Breakdown ----
  loadLeadSourceBreakdown(): void {
    this.locationAnalyticsService.getCasteLeadSourceBreakdown(this.casteId)
      .pipe(takeUntil(this.destroy$)).subscribe({
        next: (res) => this.leadSourceList = res || [],
        error: err => console.error('Error loading caste lead source breakdown:', err)
      });
  }
  ...
  // ---- Actions ----
  goBack(): void { this.router.navigate(['/location-analytics/castes']); }
  ...
  // --- Modal Control for Student Analytics ---
  ...
  viewUserBreakdownForms(userId: number, userName: string, initialTab: string): void {
    this.onViewDetails(userId, userName, initialTab, '', '', 'user');
  }

  viewConsultancyForms(id: number, name: string, initialTab: string): void {
    this.onViewDetails(id, name, initialTab, '', '', 'consultancy');
  }

  viewInstitutionForms(id: number, name: string, initialTab: string): void {
    this.onViewDetails(id, name, initialTab, '', '', 'institution');
  }

  viewCourseForms(id: number, name: string, initialTab: string): void {
    this.onViewDetails(id, name, initialTab, '', '', 'course');
  }

  viewLeadSourceForms(id: string, name: string, initialTab: string): void {
    this.onViewDetails(id, name, initialTab, '', '', 'leadSource');
  }

  onSearchStudent(tab: string, val: string): void {
    if (tab === 'remaining_applications' || tab === 'total_applications') {
      this.totalAppSearch = val;
      this.totalAppPage = 1;
    } else if (tab === 'cancelled_applications') {
      this.cancelledAppSearch = val;
      this.cancelledAppPage = 1;
    } else if (tab === 'confirmed_admissions') {
      this.totalAdmSearch = val;
      this.totalAdmPage = 1;
    } else if (tab === 'cancelled_admissions') {
      this.cancelledAdmSearch = val;
  ...
  exportStudents(tab: string): void {
    ...
    this.locationAnalyticsService.exportCasteStudents(
      this.casteId, tab, search, source, scholar, duplicateOnly, showOnlyFoc, showOnlySbs
    ).subscribe({
      next: (blob) => { this.downloadBlob(blob, `Caste_${this.casteId}_${tab}_Students.xlsx`); this.exporting[key] = false; },
  ...
  exportUsers(): void {
    ...
    this.locationAnalyticsService.exportCasteUsers(this.casteId, undefined, this.userSortBy, this.userSortDir).subscribe({
      next: (blob) => { this.downloadBlob(blob, `Caste_${this.casteId}_Users.xlsx`); this.exporting['users'] = false; },
  ...
  exportConsultancies(): void {
    ...
    this.locationAnalyticsService.exportCasteConsultancies(this.casteId, undefined, this.consSortBy, this.consSortDir).subscribe({
      next: (blob) => { this.downloadBlob(blob, `Caste_${this.casteId}_Consultancies.xlsx`); this.exporting['consultancies'] = false; },
  ...
  exportInstitutions(): void {
    ...
    this.locationAnalyticsService.exportCasteInstitutions(this.casteId, undefined, this.instSortBy, this.instSortDir).subscribe({
      next: (blob) => { this.downloadBlob(blob, `Caste_${this.casteId}_Institutions.xlsx`); this.exporting['institutions'] = false; },
  ...
  exportCourses(): void {
    ...
    this.locationAnalyticsService.exportCasteCourses(this.casteId, undefined, this.courseSortBy, this.courseSortDir).subscribe({
      next: (blob) => { this.downloadBlob(blob, `Caste_${this.casteId}_Courses.xlsx`); this.exporting['courses'] = false; },
  ...
  exportCourseTypes(): void {
    ...
    this.locationAnalyticsService.exportCasteCourseTypes(this.casteId).subscribe({
      next: (blob) => { this.downloadBlob(blob, `Caste_${this.casteId}_CourseTypes.xlsx`); this.exporting['courseTypes'] = false; },
  ...
  exportLeadSources(): void {
    ...
    this.locationAnalyticsService.exportCasteLeadSources(this.casteId).subscribe({
      next: (blob) => { this.downloadBlob(blob, `Caste_${this.casteId}_LeadSources.xlsx`); this.exporting['leadSources'] = false; },
