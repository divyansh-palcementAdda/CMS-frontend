...
@Component({
  selector: 'app-location-analytics-genders',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './location-analytics-genders.component.html',
  styleUrl: './location-analytics-genders.component.scss'
})
export class LocationAnalyticsGendersComponent implements OnInit, OnDestroy {
  gendersData: LocationAnalyticsDTO[] = [];
  ...
  loadData(): void {
    ...
    this.locationAnalyticsService.getGendersAnalytics(requestPayload).subscribe({
      next: (res) => {
        this.gendersData = res.content || [];
        this.totalElements = res.totalElements || 0;
        this.totalPages = res.totalPages || 0;
        this.loading = false;
      },
      error: (err) => {
        console.error('Error loading genders analytics', err);
        this.loading = false;
      }
    });
  }
  ...
  placeholderDownload(): void {
    alert('Excel Export is not directly supported on grouped Gender list. Please go to individual Gender detail view to export detailed user/consultancy spreadsheets.');
  }
  ...
  navigateToGenderDetail(gender: string | undefined): void {
    if (!gender) return;
    this.router.navigate(['/location-analytics/genders', gender]);
  }
}
